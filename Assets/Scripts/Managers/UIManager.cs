...
public class UIManager : MonoBehaviour
{
    ...
    public void DisplayScenario(Player p){
        if (p == GameManager.gm.p1_script){ // if for player 1
            p1_questiontxt.text = GameManager.gm.p1_currentScenario.p1_situation.question;
            //TO DO: make options appear at random buttons
            if ((p1_option1_btn.IsActive()) && (p1_option2_btn.IsActive())){
                p1_option1_btn.GetComponentInChildren<Text>().text = GameManager.gm.p1_currentScenario.p1_situation.options[0].text;
                p1_option2_btn.GetComponentInChildren<Text>().text = GameManager.gm.p1_currentScenario.p1_situation.options[1].text;
            }           
        }
            ...
            if ((p2_option1_btn.IsActive()) && (p2_option2_btn.IsActive())){
                p2_option1_btn.GetComponentInChildren<Text>().text = GameManager.gm.p2_currentScenario.p2_situation.options[0].text;
                p2_option2_btn.GetComponentInChildren<Text>().text = GameManager.gm.p2_currentScenario.p2_situation.options[1].text;
            }
        }
    }
    ...
    public void DisplayResultPanels(){
        ...
        {
            p1_resultsPanel.GetComponentInChildren<Text>().text = GameManager.gm.currentDayScenarios[0].p1_result;
            p1_showresultsAnimator.SetBool("Show", true);
            p2_resultsPanel.GetComponentInChildren<Text>().text = GameManager.gm.currentDayScenarios[0].p2_result;
            p2_showresultsAnimator.SetBool("Show", true);
            //StartDialogue(string[])
        }
    }

    public void ToggleButtons(Player p, bool b){
        if (p == GameManager.gm.p1_script){ // if for player 1
